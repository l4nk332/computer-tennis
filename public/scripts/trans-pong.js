...
function setupMenu2() {
  ...
  var childElt = document.querySelector("p");
  ...
  addEventListener("click", function (event) {
    ...
    if (name === "game-type" && id !== "tennis") {
      ...
      var soundButtons = document.querySelectorAll('[name="sound-type"]');
      ...
      var _soundButtons = document.querySelectorAll('[name="sound-type"]');
      _select.disabled = false;
      _soundButtons[0].disabled = false;
      _soundButtons[1].disabled = false;
    }
  });
}
...
// keyCodes
var controlCodes = { 37: "p1Left", 38: "p1Backhand", 39: "p1Right", 40: "p1JumpTo", 65: "p2Left", 68: "p2Right", 83: "p2JumpTo", 87: "p2Backhand" };
...
var keyEvent = trackKeys(controlCodes);
...
var Board = function Board() {
  ...
  var gameType = arguments.length <= 2 || arguments[2] === undefined ? { sfx: "normal" } : arguments[2];
...
Board.prototype.generateBounceSfx = function () {
  ...
  if (this.gameTypeSfx === "normal" || this.gameTypeSfx === "grunt") {
    for (var sfx = 1; sfx <= 5; sfx++) {
      var sfxTrack = new Audio("/assets/ball_bounce/" + sfx + ".mp3");
      this.sounds.push(sfxTrack);
    }
  } else if (this.gameTypeSfx === "classic") {
    var _sfxTrack = new Audio("/assets/classic/limit.mp3");
    this.sounds.push(_sfxTrack);
  } else if (this.gameTypeSfx === "chromatic") {
    var _sfxTrack2 = new Audio("/assets/digital/limit.mp3");
    this.sounds.push(_sfxTrack2);
  }
};
...
Board.prototype.bounceSfx = function () {
  var randomBounce = Math.floor(Math.random() * this.sounds.length);
  var randomBounceSrc = this.sounds[randomBounce];
...
*/
var Game = function Game(playerA, playerB, mainBall, gameType) {
  this.score = {
    playerOne: 0,
    playerTwo: 0
  };
...
Game.prototype.registerSpeed = function () {
  var xVelocity = this.activeMembers.balls[0].velocity.x;
  var yVelocity = this.activeMembers.balls[0].velocity.y;
  var calculatedVelocity = Math.pow(xVelocity, 2) + Math.pow(yVelocity, 2);
...
Game.prototype.registerMaxRally = function () {
  var p1CollisionCount = this.activeMembers.playerOne.collisionCount;
  var p2CollisionCount = this.activeMembers.playerTwo.collisionCount;
  var calculatedCount = Math.floor(p1CollisionCount + p2CollisionCount / 2);
...
Game.prototype.initialize = function (board) {
  ...
  function render() {
    ...
    board.space.balls.forEach(function (currentBall) {
      // Move ball and assign result to variable simultaneously
      var potentialPoint = currentBall.move(board);
      if (potentialPoint) {
        if (potentialPoint.playerPoint) {
          ...
          var pointWinner = potentialPoint.playerPoint;
          var pointLoser = pointWinner === "playerOne" ? "playerTwo" : "playerOne";
        ...
        if (potentialPoint.remove) {
          ...
          setTimeout(function () {
            var xVDirection = Math.random() < 0.5 ? -1 : 1;
            var yVDirection = potentialPoint.playerPoint === "playerOne" ? -1 : 1;
            var newBall = new Ball(board, 5, 5, 4 * xVDirection, 4 * yVDirection);
...
Game.prototype.removeBall = function (board, ball) {
  this.activeMembers.balls = this.activeMembers.balls.filter(function (currentBall) {
    return currentBall !== ball;
  });
  board.space.balls = board.space.balls.filter(function (currentBallPos) {
...
Game.prototype.declareWinner = function (player) {
  var winner = player === "playerOne" ? "one" : "two";
  ...
  var svgTrophy = document.createElement("div");
  svgTrophy.setAttribute("class", "svg-trophy");
  winMessage.appendChild(svgTrophy);
  var trophyBar = document.createElement("hr");
  ...
  if (this.gameType.court !== "chromatic" && this.gameType.court !== "classic") {
    ...
    var colors = ["#ffc26b", "#ff796b", "#5991c2", "#59d574"];
    var randColor1 = colors.splice(Math.floor(Math.random() * colors.length), 1)[0];
    var randColor2 = colors.splice(Math.floor(Math.random() * colors.length), 1)[0];
  ...
  var continueMessage = document.createElement("p");
...
var Player = function Player(board) {
  var playerNumber = arguments.length <= 1 || arguments[1] === undefined ? 1 : arguments[1];
  var width = arguments.length <= 2 || arguments[2] === undefined ? 100 : arguments[2];
  var height = arguments.length <= 3 || arguments[3] === undefined ? 25 : arguments[3];
  var gameType = arguments.length <= 4 || arguments[4] === undefined ? { sfx: "normal" } : arguments[4];
  var name = arguments.length <= 5 || arguments[5] === undefined ? "anonymous" : arguments[5];
  var photo = arguments.length <= 6 || arguments[6] === undefined ? null : arguments[6];
...
Player.prototype.moveLeft = function () {
  var velocity = arguments.length <= 0 || arguments[0] === undefined ? 5 : arguments[0];
...
Player.prototype.increaseSize = function () {
  var growth = this.initialWidth / 20;
...
Player.prototype.decreaseSize = function () {
  var shrink = this.initialWidth / 10;
...
Player.prototype.generateGruntSfx = function () {
  ...
  if (this.gameTypeSfx === "normal") {
    var fileDir = "/assets/racket/";
    for (var track = 1; track <= 10; track++) {
      var playerSfx = fileDir + track + ".mp3";
      this.sounds.push(new Audio(playerSfx));
    }
  } else if (this.gameTypeSfx === "grunt") {
    var _fileDir = "/assets/tennis_grunts/";
    for (var _track = 1; _track <= 15; _track++) {
      var _playerSfx = _fileDir + _track + ".mp3";
      this.sounds.push(new Audio(_playerSfx));
    }
  } else if (this.gameTypeSfx === "classic") {
    var _playerSfx2 = new Audio("/assets/classic/paddel.mp3");
    this.sounds.push(_playerSfx2);
  } else if (this.gameTypeSfx === "chromatic") {
    var playerSfx1 = new Audio("/assets/digital/paddel_1.mp3");
    var playerSfx2 = new Audio("/assets/digital/paddel_2.mp3");
...
Player.prototype.gruntSfx = function () {
  var randomGrunt = Math.floor(Math.random() * this.sounds.length);
  var randomGruntSrc = this.sounds[randomGrunt];
  // This ensures the audio api doesn't crash (if volume > 1)
  this.volumeLevel += this.volumeLevel < 1 ? 0.5 : 0;
...
*/
var Ball = function Ball(board) {
  var width = arguments.length <= 1 || arguments[1] === undefined ? 15 : arguments[1];
  var height = arguments.length <= 2 || arguments[2] === undefined ? 15 : arguments[2];
  var velocityX = arguments.length <= 3 || arguments[3] === undefined ? 5 : arguments[3];
  var velocityY = arguments.length <= 4 || arguments[4] === undefined ? 5 : arguments[4];
...
Ball.prototype.intersect = function (lineOne, lineTwo) {
  var x1 = lineOne.start.x;
  var y1 = lineOne.start.y;
  var x2 = lineOne.end.x;
  var y2 = lineOne.end.y;
  var x3 = lineTwo.start.x;
  var y3 = lineTwo.start.y;
  var x4 = lineTwo.end.x;
  var y4 = lineTwo.end.y;

  var pxTop = (x1 * y2 - y1 * x2) * (x3 - x4) - (x1 - x2) * (x3 * y4 - y3 * x4);
  var pxBottom = (x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4);
  var px = pxTop / pxBottom;

  var pyTop = (x1 * y2 - y1 * x2) * (y3 - y4) - (y1 - y2) * (x3 * y4 - y3 * x4);
  var pyBottom = (x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4);
  var py = pyTop / pyBottom;
...
Ball.prototype.moveBtmRtCorner = function (board) {
  var projectileVectorA = void 0,
      projectileVectorB = void 0;
  var ballBottomLeftCorner = { x: this.position.x, y: this.position.y + this.height };
  var ballBottomRightCorner = { x: this.position.x + this.width, y: this.position.y + this.height };
  // p1 (top line)
  var p1 = {
    start: {
  ...
  var p1IntersectionA = this.intersect(projectileVectorA, p1);
  var p1IntersectionB = this.intersect(projectileVectorB, p1);
  var limitRightIntersectionA = this.intersect(projectileVectorA, this.limits.right);
...
Ball.prototype.moveTopRtCorner = function (board) {
  var projectileVectorA = void 0,
      projectileVectorB = void 0;
  var ballTopLeftCorner = { x: this.position.x, y: this.position.y };
  var ballTopRightCorner = { x: this.position.x + this.width, y: this.position.y };
  // p2 (bottom line)
  var p2 = {
    start: {
  ...
  var p2IntersectionA = this.intersect(projectileVectorA, p2);
  var p2IntersectionB = this.intersect(projectileVectorB, p2);
  var limitRightIntersectionA = this.intersect(projectileVectorA, this.limits.right);
...
Ball.prototype.moveBtmLtCorner = function (board) {
  ...
  var p1IntersectionA = this.intersect(projectileVectorA, p1);
  var p1IntersectionB = this.intersect(projectileVectorB, p1);
  var limitLeftIntersectionA = this.intersect(projectileVectorA, this.limits.left);
...
Ball.prototype.increaseVelocity = function () {
  var increment = arguments.length <= 0 || arguments[0] === undefined ? 0.25 : arguments[0];
...
function init() {
  ...
  window.addEventListener("keydown", function (event) {
    if (event.keyCode == "32" && menuExit === 0) {
      ...
      // Show second menu
      var gameSettings = void 0;
      var tennisMode = document.getElementById("tennis");
      var classicMode = document.getElementById("classic");
      var chromaticMode = document.getElementById("chromatic");
      if (tennisMode.checked) {
        var sfx = void 0;
        var normalSfx = document.getElementById("normal");
        var gruntSfx = document.getElementById("grunt");
        ...
        var courtValue = courtSelect.options[courtSelect.selectedIndex].value;
        gameSettings = {
          mode: "tennis",
          sfx: sfx,
          court: courtValue
      ...
      container.innerHTML = "";
      /*============= Game Jumpstart Code Below =============*/
      var pongBoard = new Board(400, 600, gameSettings);
      var firstPlayer = new Player(pongBoard, 1, 100, 5, gameSettings);
      var secondPlayer = new Player(pongBoard, 2, 100, 5, gameSettings);
      var pongBall = new Ball(pongBoard, 5, 5, 4, 4);
      var pongGame = new Game(firstPlayer, secondPlayer, pongBall, gameSettings);
...
init();
//# sourceMappingURL=trans-pong.js.map
